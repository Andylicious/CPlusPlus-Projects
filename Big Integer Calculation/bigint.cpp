...
bigint bigint::operator+ (const bigint &that) const {
   ...
   if ( this->negative == that.negative ){
      bigadd(this->big_value, that.big_value, result.big_value);
      result.set_negative(that.negative);
   }else {
      int abs_compare = abscompare(that);
      if ( abs_compare > 0 ){
         bigsub(this->big_value, that.big_value, result.big_value);
         result.set_negative(negative);
      }else if ( abs_compare < 0 ){
         bigsub(that.big_value, this->big_value, result.big_value);
         result.set_negative(that.negative);
      }
   }
...
bigint bigint::operator- (const bigint &that) const {
   ...
   if ( negative == that.negative ){
      int abs_compare = abscompare(that);
      if ( abs_compare > 0 ){
         bigsub(big_value, that.big_value, result.big_value);
         result.set_negative(this->negative?true:false);
      }else if ( abs_compare < 0){
         bigsub(that.big_value, big_value, result.big_value);
         result.set_negative(negative?false:true);
      }
   }else {
      bigadd( big_value, that.big_value, result.big_value);
...
bigint bigint::mul_by_2 () {
   bigint result = bigint();
   bigadd(big_value,big_value,result.big_value);
   return result;
}

//a is bigger than b
void bigint::bigadd(const bigvalue_t* a,
                       const bigvalue_t* b,
...
//a is bigger than b
void bigint::bigsub(const bigvalue_t* a,
                       const bigvalue_t* b,
                       bigvalue_t* target) const{
   int carry = 0;
   bool a_done = false, b_done = false;
...
bigint bigint::pow (const bigint &that) const {
    ...
    */
   if (that > 999) throw range_error ("extraordinary ");
